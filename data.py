...
def get_new4_3tuple(ids, images, coords_in_cell, matrix):
    ...
    # print(len(n1_image))
    # print(n1_image)
    n2_index = list(map(lambda item: item[2], ids))
    # print('n0_index:')
    # print(len(n0_index))
    # print(n0_index)
    n2_image = list(map(lambda item: item[2], images))
    # print('n0_image:')
    # print(len(n0_image))
    # print(n0_image)
    n3_index = list(map(lambda item: item[3], ids))
    # print('n1_index:')
    # print(len(n1_index))
    # print(n1_index)
    n3_image = list(map(lambda item: item[3], images))
    all_pos_ij = []
    all_pos_in0 = []
    all_pos_in1 = []
    all_pos_in2 = []
    ...
    for i in range(len(ids)):
        ...
        # print(pos_in1)
        n2_index_incell_i = n2_index[i]
        # print(n0_index_incell_i)
        n2_image_i = n2_image[i]
        # print(n0_image_i)
        shifted = np.sum(matrix * n2_image_i[:, np.newaxis], axis=0)
        pos_n2 = shifted + coords_in_cell[n2_index_incell_i]
        # print(pos_n0)
        pos_in2 = pos_n2 - pos_i
        # print(pos_in0)
        n3_index_incell_i = n3_index[i]
        # print(n1_index_incell_i)
        n3_image_i = n3_image[i]
        # print(n1_image_i)
        shifted = np.sum(matrix * n3_image_i[:, np.newaxis], axis=0)
        pos_n3 = shifted + coords_in_cell[n3_index_incell_i]
        # print(pos_n1)
        pos_in3 = pos_n3 - pos_i
        # print(pos_in1)
        ...
        for index_j, j in enumerate(ids[i]):
            ...
            n2_index_incell_j = n2_index[j]
            # print('n0_index_incell_j')
            # print(n0_index_incell_j)
            n2_image_j = n2_image[j] + images[i][index_j]
            # print('n0_image_j')
            # print(n0_image_j)
            shifted = np.sum(matrix * n2_image_j[:, np.newaxis], axis=0)
            pos_n2j = shifted + coords_in_cell[n2_index_incell_j]
            # print('pos_n0j')
            # print(pos_n0j)
            pos_jn2 = pos_n2j - pos_j
            # print('pos_jn0')
            # print(pos_jn0)
            n3_index_incell_j = n3_index[j]
            n3_image_j = n3_image[j] + images[i][index_j]
            shifted = np.sum(matrix * n3_image_j[:, np.newaxis], axis=0)
            pos_n3j = shifted + coords_in_cell[n3_index_incell_j]
            pos_jn3 = pos_n3j - pos_j
...
def load_infinite_graphs(
    ...
    def atoms_to_graph(atoms):
        ...
        tuple_u, tuple_v, dist, theta, phi, tau = get_new3_3tuple(all_index, all_images, coords_in_cell, matrix)
        # tuple_edge_index, dist, theta, phi, tau = get_4_tuple(structure.cart_coords, new_cutoff=4.0)
        # print(test done)
        data = Data(x=node_features, edge_attr=r.norm(dim=-1), edge_index=torch.stack([u, v]),
                    inf_edge_index=edge_index,
