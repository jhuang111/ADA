...
class ADAGNNConfig(BaseSettings):
    name: Literal["ADAGNN"]
    conv_layers: int = 3
...
class ADAGNNConv(MessagePassing):
    ...
    def __init__(self, fc_features,
                 ...
                 output_channels=256,
                 act=swish):
        super(ADAGNNConv, self).__init__(node_dim=0)
...
class ADAGNN(nn.Module):
    ...
    def __init__(self, config: ADAGNNConfig = ADAGNNConfig(name="ADAGNN")):
        super().__init__()
        self.config = config
    ...
    def forward(self, data, print_data=False):
        ...
        edge_index = torch.cat([edge_index, tuple_index], 1)
        edge_features = torch.cat([edge_features, feature1], 0)
